      ...
      <form action="{{ route('pengaturan.ubah-profile') }}" method="post">
        ...
        <div class="card-body">
          @if (Auth::user()->role == "Ustadz")
            <div class="row">
              <input type="hidden" name="role" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->role }}">
              <div class="col-md-6">
                  <div class="form-group">
                      <label for="name">Nama Ustadz/Ustadzah</label>
                      ...
                      <select id="mapel_id" name="mapel_id" class="select2bs4 form-control @error('mapel_id') is-invalid @enderror">
                          ...
                          @foreach ($mapel as $data)
                              <option value="{{ $data->id }}"
                                  @if (Auth::user()->ustadz(Auth::user()->id_card)->mapel_id == $data->id)
                                      selected
                                  @endif
                  ...
                  <div class="form-group">
                      ...
                      <input type="text" id="tmp_lahir" name="tmp_lahir" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->tmp_lahir }}" class="form-control @error('tmp_lahir') is-invalid @enderror">
                  </div>
                  <div class="form-group">
                      <label for="id_card">Nomor ID Card</label>
                      <input type="text" id="id_card" name="id_card" class="form-control" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->id_card }}" disabled>
                  </div>
                  <div class="form-group">
                      <label for="telp">Nomor Telpon/HP</label>
                      <input type="text" id="telp" name="telp" onkeypress="return inputAngka(event)" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->telp }}" class="form-control @error('telp') is-invalid @enderror">
                  </div>
              </div>
              <div class="col-md-6">
                  <div class="form-group">
                      <label for="nip">NIP</label>
                      <input type="text" id="nip" name="nip" onkeypress="return inputAngka(event)" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->nip }}" class="form-control @error('nip') is-invalid @enderror" disabled>
                  </div>
                  <div class="form-group">
                      ...
                      <select id="jk" name="jk" class="select2bs4 form-control @error('jk') is-invalid @enderror">
                          ...
                          <option value="L"
                              @if (Auth::user()->ustadz(Auth::user()->id_card)->jk == 'L')
                                  selected
                              @endif
                          >Laki-Laki</option>
                          <option value="P"
                              @if (Auth::user()->ustadz(Auth::user()->id_card)->jk == 'P')
                                  selected
                              @endif
                          >Perempuan</option>
                      </select>
                  ...
                  <div class="form-group">
                      ...
                      <input type="date" id="tgl_lahir" name="tgl_lahir" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->tgl_lahir }}" class="form-control @error('tgl_lahir') is-invalid @enderror">
                  </div>
                  <div class="form-group">
                      <label for="kode">Kode Jadwal</label>
                      <input type="text" id="kode" name="kode" class="form-control" value="{{ Auth::user()->ustadz(Auth::user()->id_card)->kode }}" disabled>
                  </div>
              </div>
            </div>
